...
use crate::{
    ...
    traits::{next_trait_solve, trait_solve_query, FnTrait, NextTraitSolveResult},
...
impl<'a> InferenceTable<'a> {
    ...
    pub(crate) fn normalize_ty(
        ...
        let proj_ty =
            chalk_ir::ProjectionTy { associated_ty_id: assoc_type_id, substitution: subst.clone() };
        let alias = chalk_ir::AliasTy::Projection(proj_ty);
        let var = self.new_type_var();
        let goal =
            chalk_ir::GoalData::DomainGoal(chalk_ir::DomainGoal::Normalize(chalk_ir::Normalize {
                alias,
                ty: var,
            }))
            .intern(Interner);
