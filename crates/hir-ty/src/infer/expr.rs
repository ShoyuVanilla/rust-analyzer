...
use hir_def::{
    ...
    resolver::ValueNs,
    BlockId, ClosureLoc, CoroutineLoc, FieldId, GenericDefId, GenericParamId, ItemContainerId,
    Lookup, OpaqueTyLoc, TupleFieldId, TupleId,
};
...
use crate::{
    ...
    mapping::{
        from_chalk, from_chalk_closure_id, to_chalk_closure_id, to_chalk_coroutine_id,
        to_opaque_ty_id, ToChalk,
    },
            ...
            Expr::Closure { body, args, ret_type, arg_types, closure_kind, capture_by: _ } => {
                ...
                let (id, ty, resume_yield_tys) = match closure_kind {
                    ClosureKind::Coroutine(_) => {
                        ...
                        let coroutine_id = self.db.intern_coroutine_def(CoroutineLoc {
                            parent: self.owner,
                            root: tgt_expr,
                        });
                        let coroutine_ty =
                            TyKind::Coroutine(to_chalk_coroutine_id(coroutine_id), subst)
                                .intern(Interner);
                        ...
                        self.table.resolve_completely(callee_ty.clone()).kind(Interner)
                    {
                        if let Some(par) = self.current_closure {
                            self.closure_dependencies
                                .entry(par)
                                .or_default()
                                .push(from_chalk_closure_id(*c));
                        }
                        self.deferred_closures.entry(from_chalk_closure_id(*c)).or_default().push(
                            (derefed_callee.clone(), callee_ty.clone(), params.clone(), tgt_expr),
                        );
                    }
