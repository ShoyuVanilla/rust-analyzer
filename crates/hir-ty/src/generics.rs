...
pub(crate) fn parent_generic_def(db: &dyn DefDatabase, def: GenericDefId) -> Option<GenericDefId> {
    let container = match def {
        ...
        GenericDefId::CoroutineId(it) => return it.lookup(db).parent.as_generic_def_id(db),
        GenericDefId::OpaqueTyId(it) => {
            return match it.lookup(db) {
                hir_def::OpaqueTyLoc::ReturnTypeImplTrait(function_id, _) => {
                    Some(function_id.into())
                }
                hir_def::OpaqueTyLoc::TypeAliasImplTrait(_, _) => None,
                hir_def::OpaqueTyLoc::AsyncBlockTypeImplTrait(def_with_body_id, _) => {
                    def_with_body_id.as_generic_def_id(db)
                }
            }
        }
