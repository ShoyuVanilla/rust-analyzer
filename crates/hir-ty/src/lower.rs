...
use chalk_ir::{
    ...
    DomainGoal, Mutability, Safety, TypeOutlives,
};
...
use hir_def::{
    ...
    Lookup, OpaqueTyLoc, StaticId, StructId, TypeAliasId, TypeOrConstParamId, TypeOwnerId, UnionId,
    VariantId,
};
...
use crate::{
    ...
    FnPointer, FnSig, FnSubst, ImplTrait, ImplTraits, Interner, Lifetime, LifetimeData,
    LifetimeOutlives, ParamKind, PolyFnSig, QuantifiedWhereClause, QuantifiedWhereClauses,
            ...
            TypeRef::ImplTrait(bounds) => {
                match self.impl_trait_mode.mode {
                    ImplTraitLoweringMode::Opaque => {
                        ...
                        TyKind::OpaqueType(to_opaque_ty_id(opaque_ty_id), parameters)
                            .intern(Interner)
                    }
...
pub(crate) fn trait_environment_query(
    ...
    for (params, def) in resolver.all_generic_params() {
        ...
        for pred in params.where_predicates() {
            for pred in ctx.lower_where_predicate(pred, def, false) {
                ...
                let program_clause: Binders<DomainGoal<Interner>> =
    ...
    if let Some(trait_id) = def.assoc_trait_container(db.upcast()) {
        ...
        clauses.push(Binders::empty(
            Interner,
            pred.cast::<DomainGoal<Interner>>(Interner).into_from_env_goal(Interner),
        ));
    }
    ...
    if !subst.is_empty(Interner) {
            ...
            implicitly_sized_clauses(db, def, &explicitly_unsized_tys, &subst, &resolver)
        {
            clauses.extend(implicitly_sized_clauses.map(|pred| {
                Binders::empty(
                    Interner,
                    pred.into_from_env_goal(Interner).cast::<DomainGoal<Interner>>(Interner),
                )
            }));
        };
    }
