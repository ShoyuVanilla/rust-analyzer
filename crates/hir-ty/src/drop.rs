...
use crate::{from_chalk_closure_id, ConcreteConst, ConstScalar, ConstValue};
...
pub(crate) fn has_drop_glue(db: &dyn HirDatabase, ty: Ty, env: Arc<TraitEnvironment>) -> DropGlue {
    match ty.kind(Interner) {
        ...
        TyKind::Closure(closure_id, subst) => {
            let closure_id = from_chalk_closure_id(*closure_id);
            let owner = db.lookup_intern_closure_def(closure_id).parent;
