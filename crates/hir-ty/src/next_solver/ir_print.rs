    ...
    fn print_debug(t: &ty::AliasTy<Self>, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        crate::next_solver::tls::with_opt_db_out_of_thin_air(|db| match db {
            Some(db) => match t.def_id {
                hir_def::GenericDefId::TypeAliasId(id) => fmt.write_str(&format!(
                    "AliasTy({:?}[{:?}])",
                    db.type_alias_data(id).name.as_str(),
                    t.args
                )),
                hir_def::GenericDefId::OpaqueTyId(id) => {
            ...
            Some(db) => {
                ...
                if trait_args.is_empty() {
                    fmt.write_str(&format!(
                        "{:?}: {}",
                        self_ty,
                        db.trait_data(trait_).name.as_str()
                    ))
                } else {
                    fmt.write_str(&format!(
                        "{:?}: {}<{:?}>",
