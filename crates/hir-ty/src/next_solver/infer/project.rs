...
use crate::next_solver::{infer::snapshot::undo_log::InferCtxtUndoLogs, AliasTerm, ParamEnv, Term};
...
pub type NormalizedTerm = Normalized<Term>;

impl<T> Normalized<T> {
    pub fn with<U>(self, value: U) -> Normalized<U> {
...
pub struct ProjectionCache<'a> {
    map: &'a mut SnapshotMapStorage<ProjectionCacheKey, ProjectionCacheEntry>,
    undo_log: &'a mut InferCtxtUndoLogs,
}

#[derive(Clone, Default)]
pub struct ProjectionCacheStorage {
    map: SnapshotMapStorage<ProjectionCacheKey, ProjectionCacheEntry>,
}
...
impl ProjectionCacheKey {
    pub fn new(term: AliasTerm, param_env: ParamEnv) -> Self {
        Self { term, param_env }
    }
}

#[derive(Clone, Debug)]
pub enum ProjectionCacheEntry {
    InProgress,
    Ambiguous,
    Recur,
    Error,
...
impl ProjectionCache<'_> {
    ...
    pub fn insert_term(&mut self, key: ProjectionCacheKey, value: NormalizedTerm) {
        debug!(
    ...
    pub fn complete(&mut self, key: ProjectionCacheKey, result: EvaluationResult) {
        let mut map = self.map();
        match map.get(&key) {
    ...
    pub fn is_complete(&mut self, key: ProjectionCacheKey) -> Option<EvaluationResult> {
    ...
    /// be different).
    pub fn ambiguous(&mut self, key: ProjectionCacheKey) {
        let fresh = self.map().insert(key.clone(), ProjectionCacheEntry::Ambiguous);
    ...
    /// an error here.
    pub fn recur(&mut self, key: ProjectionCacheKey) {
        let fresh = self.map().insert(key.clone(), ProjectionCacheEntry::Recur);
        assert!(!fresh, "never started projecting `{key:?}`");
    }

    /// Indicates that trying to normalize `key` resulted in
    /// error.
    pub fn error(&mut self, key: ProjectionCacheKey) {
        let fresh = self.map().insert(key.clone(), ProjectionCacheEntry::Error);
        assert!(!fresh, "never started projecting `{key:?}`");
    }
}

impl Rollback<ProjectUndoLog> for ProjectionCacheStorage {
    fn reverse(&mut self, undo: ProjectUndoLog) {
