...
use hir_def::{
    ...
    AdtId, AssocItemId, AssocItemLoc, AttrDefId, CallableDefId, ClosureId, ConstId, ConstParamId,
...
use hir_ty::{
    ...
    direct_super_traits, error_lifetime, from_chalk_closure_id, known_const_to_ast,
    ...
    AliasTy, CallableSig, Canonical, CanonicalVarKinds, Cast, GenericArg, GenericArgData, Interner,
    ParamKind, QuantifiedWhereClause, Scalar, Substitution, TraitEnvironment, TraitRefExt, Ty,
    TyBuilder, TyDefId, TyExt, TyKind, TyLoweringDiagnostic, ValueTyDefId, WhereClause,
};
            ...
            Callee::Closure(closure, _) => {
                ...
                if let Expr::Closure { args, .. } = &body[c.root] {
                    if let Pat::Bind { id, .. } = &body[args[self.idx]] {
                        return Some(Local { parent: c.parent, binding_id: *id });
                    }
                }
                None
            }
    ...
    pub fn diagnostics(self, db: &dyn HirDatabase, acc: &mut Vec<AnyDiagnostic>) {
        ...
        let source_map = match &generics_source_map {
            ...
            None => match def {
                ...
                | GenericDefId::ClosureId(_)
                | GenericDefId::CoroutineId(_)
                | GenericDefId::OpaqueTyId(_)
                | GenericDefId::Ctor(_) => return,
            },
        };
...
impl Type {
    ...
    pub fn as_callable(&self, db: &dyn HirDatabase) -> Option<Callable> {
        let callee = match self.ty.kind(Interner) {
            TyKind::Closure(id, subst) => {
                Callee::Closure(from_chalk_closure_id(*id), subst.clone())
            }
    ...
    pub fn as_closure(&self) -> Option<Closure> {
        match self.ty.kind(Interner) {
            TyKind::Closure(id, subst) => {
                Some(Closure { id: from_chalk_closure_id(*id), subst: subst.clone() })
            }
            _ => None,
        }
    }
