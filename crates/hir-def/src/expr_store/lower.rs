...
impl ExprCollector<'_> {
    ...
    fn collect_format_args(
        ...
        let use_format_args_new = fmt_args.is_some() && fmt_unsafe_arg.is_none();

        let idx = if use_format_args_new {
            self.collect_format_args_impl_new(
                syntax_ptr,
                fmt,
                hygiene,
                argmap,
    ...
    fn collect_format_args_impl_new(
        &mut self,
        syntax_ptr: AstPtr<ast::Expr>,
        fmt: FormatArgs,
        hygiene: HygieneId,
        ...
        let call_block = {
            ...
            Expr::Unsafe { id: None, statements, tail: Some(call) }
        };
